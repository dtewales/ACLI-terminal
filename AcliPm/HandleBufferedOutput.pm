...
sub matchline { # Does port and vlan range expansion if necessary, and then does the match against grep string
	...
	if ($grep->{Instance}[$g] && $grep->{Instance}[$g] eq 'Port' && $line =~ /(?:\d\-\d|\d\/ALL)/) {
		...
		#1 /\s\K((?:\d{1,2}\/\d{1,2}(?:\-\d{1,2}\/\d{1,2})?,)*  \d{1,2}\/\d{1,2}-\d{1,2}\/\d{1,2} (?:,\d{1,2}\/\d{1,2}(?:\-\d{1,2}\/\d{1,2})?)*)(?=\s|$)/  = modular
		#1b/\s\K((?:\d{1,2}\/\d{1,2}(?:\/\d{1,2})?(?:\-\d{1,2}\/\d{1,2}(?:\/\d{1,2})?)?,)*  \d{1,2}\/\d{1,2}(?:\/\d{1,2})?-\d{1,2}\/\d{1,2}(?:\/\d{1,2})? (?:,\d{1,2}\/\d{1,2}(?:\/\d{1,2})?(?:\-\d{1,2}\/\d{1,2}(?:\/\d{1,2})?)?)*)(?=\s|$)/  = modular (channelized support)
		#1c/\s\K((?:\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)(?:\-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)?),)*  \d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?) (?:,\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)(?:\-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?))?)*)(?=\s|$)/  = modular (channelized + insight support)
		#2 /\s\K((?:\d{1,2}\/(?:ALL|\d{1,2}(?:\-\d{1,2}  )?),)* \d{1,2}\/(?:ALL|\d{1,2}-\d{1,2})  (?:,\d{1,2}\/(?:ALL|\d{1,2}(?:\-\d{1,2}  )?))*)(?=\s|$)/ = stackable
		#3 /\s\K((?:\d{1,2}: (?:\d{1,2}(?:\-\d{1,2}      )?),)* \d{1,2}: (?:\d{1,2}-\d{1,2})      (?:,\d{1,2}: (?:\d{1,2}(?:\-\d{1,2}      )?))*)(?=\s|$)/ = XOS stack/chassis
		#4 /\s\K((?:\d{1,2}           (?:\-\d{1,2}       )?, )* \d{1,2}\        -\d{1,2}          (?:,\d{1,2}         (?:\-\d{1,2}         )?) *)(?=\s|$)/ = standalone

		#1c
		if ($line =~ s/\s\K((?:\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)(?:\-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)?),)*\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)(?:,\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?)(?:\-\d{1,2}\/(?:s\d|\d{1,2}(?:\/\d{1,2})?))?)*)(?=\s|$)/generatePortList($host_io, $1, $grep->{RangeList}[$g])/e) {
